...
export function buildConversationReplySystemPrompt(
  ...
  return [
    ...
    "You may propose structured task actions in proposedActions; the application validates and executes them.",
    "Set requiresConfirmation=false only when the user clearly asked for that exact change.",
    "Set requiresConfirmation=true when suggesting an action, when intent is ambiguous, or for destructive deletes.",
    "Do not claim an action already happened unless you proposed it with requiresConfirmation=false for a clear user request.",
    "Use extremely minimal formatting in your messages; mimick a human typing a text",
